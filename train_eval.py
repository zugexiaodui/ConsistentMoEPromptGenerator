...
def get_args():
    ...
    parser.add_argument('--moe_num_experts', type=int, default=36)
    parser.add_argument('--moe_topk', type=int, default=4)
    parser.add_argument('--moe_nosiy_gating', type=misc.str2bool, default=True)
    parser.add_argument('--moe_impt_coef', type=float, default=0.05)
    parser.add_argument('--moe_frozen_qx_coef', type=float, default=0.)
    ...
    parser.add_argument('--null_eta2', type=float, default=0.99)
    ...
    parser.add_argument('--null_use_all_seen', type=misc.str2bool, default=False)
    ...
    parser.add_argument('--sample_type', type=str, choices=('path', 'image'), default='path')
    ...
    parser.add_argument('--optimizer', type=str, default='mod_adam', help="Default: mod_adam")
...
def train_one_epoch(GVM: GlobalVarsManager, curr_epoch: int, dataloader: DataLoader, model: VisionTransformer, criterion: nn.CrossEntropyLoss, optimizer: torch.optim.Optimizer) -> str:
    ...
    if args.moe_impt_coef > 0:
        moe_loss_list = []

        def _forward_hook(module: nn.Module, args: tuple[Tensor], output: Tensor):
            if isinstance(module, utils.vit_builder.IntermReader):
                _mname = module.module_name
                _interm_tensor: Tensor = args[0]
                if _mname == 'interm_reader_loss':
                    moe_loss_list.append(_interm_tensor)
            else:
                raise NotImplementedError()

        _handle_list: list[torch.utils.hooks.RemovableHandle] = []
        for n, m in model.named_modules():
            if 'interm_reader_loss' in n and isinstance(m, utils.vit_builder.IntermReader):
    ...
    for i_batch, (images, target) in tqdm.tqdm(enumerate(dataloader, 1), total=len(dataloader), dynamic_ncols=True, disable=not GVM.args.show_bar):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=use_amp):
            if args.moe_frozen_qx_coef > 0:
                q_x = GVM.cache_dict['pretrained_model'].encode_image(mix_img, pre_logits=True).detach()
                logits: Tensor = model((mix_img, q_x))
            else:
        ...
        if i_batch == 1:
            ...
            if args.moe_impt_coef > 0:
                assert len(moe_loss_list) == args.prompt_end_block - args.prompt_start_block + 1, f"{len(moe_loss_list)}"

        ce_loss: Tensor = criterion(logits / temperature, mix_lbl)
        loss: Tensor = ce_loss.clone()

        if args.moe_impt_coef > 0:
            moe_loss: Tensor = torch.zeros_like(loss)
            for _l in moe_loss_list:
                moe_loss += _l
            moe_loss /= len(moe_loss_list)
            assert moe_loss.requires_grad
            loss += moe_loss * args.moe_impt_coef
...
def evaluate_one_task(GVM: GlobalVarsManager, train_taskid: int, eval_taskid: int, eval_task_classes: list[int], model: VisionTransformer) -> OrderedDict[str, float]:
    ...
    for images, target in tqdm.tqdm(dataloader, total=len(dataloader), dynamic_ncols=True, disable=not GVM.args.show_bar):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=use_amp):
            with torch.no_grad():
                if args.moe_frozen_qx_coef > 0:
                    q_x = GVM.cache_dict['pretrained_model'].encode_image(images, pre_logits=True).detach()
                    logits: Tensor = model((images, q_x))
                else:
...
def get_interm_tensor_dict(GVM: GlobalVarsManager, model: VisionTransformer, null_param_id_dict: dict) -> dict[int, Tensor]:
    ...
    def _forward_hook(module: nn.Module, args: tuple[Tensor], output: Tensor):
        if isinstance(module, nn.Linear):
            ...
            _mname = module.module_name
            _interm_tensor: Tensor = args[0].detach().clone()

            if _mname == 'interm_reader_1':
                assert _interm_tensor.ndim == 2, _interm_tensor.shape
            ...
            if _mname in ('interm_reader_1', 'interm_reader_2') and _pid in null_param_id_dict.keys():
    ...
    if args.null_use_all_seen:
        val_dataset = define_dataset(GVM, GVM.cl_mngr.current_task_classes, training=False, transform_type=args.transform_type, target_map_to_local=args.seperate_head, use_eval_transform=True, expand_times=1, verbose=False)
    ...
    for nb, (img, _) in enumerate(dataloader, 1):
        with torch.no_grad():
            img: Tensor
            if args.moe_frozen_qx_coef > 0:
                q_x = GVM.cache_dict['pretrained_model'].encode_image(img.cuda(non_blocking=True), pre_logits=True).detach()
                logits: Tensor = model((img.cuda(non_blocking=True), q_x))
            else:
                logits: Tensor = model(img.cuda(non_blocking=True))
    ...
    assert set(list(interm_tensor_dict.keys())) == set(list(null_param_id_dict.keys())), f"({len(interm_tensor_dict.keys())}){interm_tensor_dict.keys()}\n!=({len(null_param_id_dict.keys())}){null_param_id_dict}"
...
def get_update_projection_dict(GVM: GlobalVarsManager, null_param_id_dict: dict, interm_tensor_dict: dict[int, dict[str, Tensor]]) -> dict[int, dict[str, Tensor]]:
    ...
    for _pid in interm_tensor_dict.keys():
        ...
        for _mname in interm_tensor_dict[_pid].keys():
            U, S, Vt = torch.linalg.svd(interm_tensor_dict[_pid][_mname], full_matrices=True)
            S: Tensor
            Vt: Tensor
...
def refine_head(GVM: GlobalVarsManager, model: VisionTransformer):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        dataloader = DataLoader(train_data, batch_size=_ns, shuffle=True)
...
if __name__ == "__main__":
    ...
    for taskid, current_task_classes in GVM.cl_mngr:
        ...
        if not args.consecutive_training or taskid == 0:
            ...
            _moe_args_dict = misc.get_specific_args_dict(args, 'moe_')
            ...
            model: VisionTransformer = timm.create_model(args.model, pretrained=True, pretrained_strict=False, **_head_dim_arg_dict,
                                                         prompt_args_dict=_prompt_args_dict, moe_args_dict=_moe_args_dict, other_args_dict=_other_args_dict)
            GVM.cache_dict['pretrained_cfg'] = deepcopy(model.pretrained_cfg)

            if args.moe_frozen_qx_coef > 0:
                _prompt_args_dict['prompt_len'] = 0
                pretrained_model = timm.create_model(args.model, pretrained=True, pretrained_strict=False,
                                                     prompt_args_dict=_prompt_args_dict, moe_args_dict=_moe_args_dict,
                                                     other_args_dict=_other_args_dict)
                GVM.cache_dict['pretrained_model'] = pretrained_model

        if args.consecutive_training and taskid > 0:
            pass

        # Load pretrained prompts
