...
from utils.moe_builder import SparseDispatcher, MoE
...
class MoEPrompt(MoE):
    def __init__(self, embed_dim: int, prompt_args: dict, num_experts: int, topk: int, noisy_gating: bool, impt_coef: float, frozen_qx_coef: float):
        _pa = prompt_args
        super().__init__(embed_dim, num_experts, topk, noisy_gating)
        self.experts = nn.Parameter(torch.randn(num_experts, _pa['prompt_len'], embed_dim))
        nn.init.uniform_(self.experts, -1, 1)
        self.prompt_len: int = prompt_args['prompt_len']
        self.embed_dim = embed_dim
        self.impt_coef = impt_coef
        assert 0 <= frozen_qx_coef <= 1, frozen_qx_coef
        self.frozen_qx_coef = frozen_qx_coef
    ...
    def forward(self, x: Tensor, q_x: Tensor):
        assert x.shape[0] == q_x.shape[0] and x.shape[-1] == q_x.shape[-1] and q_x.ndim == 2, f"{x.shape}; {q_x.shape}"

        if not self.training and hasattr(self, (_reader := 'interm_reader_1')):
            q_x = getattr(self, _reader)(q_x)

        gates, load = self.noisy_top_k_gating(q_x, self.training)

        if not self.training and hasattr(self, (_reader := 'interm_reader_2')):
            gates = getattr(self, _reader)(gates)

        if self.impt_coef != 0:
            importance = gates.sum(0)
            loss = self.cv_squared(importance) + self.cv_squared(load)
            self.interm_reader_loss(loss)
        ...
        expert_outputs = [repeat(self.experts[i], 'L D -> b (L D)', b=expert_inputs[i].shape[0]) for i in range(self.num_experts)]
        batch_prompts = dispatcher.combine(expert_outputs)
        batch_prompts = rearrange(batch_prompts, 'B (M D) -> B M D', M=self.prompt_len, D=self.embed_dim)
        return batch_prompts

    def extra_repr(self) -> str:
        return f"num_experts={self.num_experts}, topk={self.topk}, noisy_gating={self.noisy_gating}, impt_coef={self.impt_coef}, frozen_qx_coef={self.frozen_qx_coef}"
...
class Block(nn.Module):
    def __init__(
            ...
            moe_prompt: MoEPrompt = None
    ) -> None:
        super().__init__()

        if moe_prompt is not None:
            self.moe_prompt = moe_prompt
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        src_num_tokens = x.shape[1] if isinstance(x, Tensor) else x[0].shape[1]
        if hasattr(self, 'moe_prompt'):
            if isinstance(x, (tuple, list)):
                x, q_x = x
                q_x: Tensor
                assert not q_x.requires_grad
                assert self.moe_prompt.frozen_qx_coef > 0
                assert q_x.shape == x[:, 0].shape
                q_x = self.moe_prompt.frozen_qx_coef * q_x + (1 - self.moe_prompt.frozen_qx_coef) * x[:, 0]
            else:
                assert isinstance(x, Tensor)
                assert self.moe_prompt.frozen_qx_coef == 0
                q_x = x[:, 0]
            _batch_prompts: Tensor = self.moe_prompt(x, q_x)

            _x_p = torch.cat([x, _batch_prompts], dim=1)
            x_a = self.attn(self.norm1(_x_p))[:, :self.pre_tokens]
        else:
        ...
        assert x.shape[1] == src_num_tokens, f"{src_num_tokens}; {x.shape}"
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            adapter_args_dict: dict[str, Any] = {},
            moe_args_dict: dict[str, Any] = {},
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                act_layer=act_layer,
                mlp_layer=mlp_layer,
                moe_prompt=MoEPrompt(embed_dim, _pd, _md['moe_num_experts'], _md['moe_topk'], _md['moe_nosiy_gating'], _md['moe_impt_coef'],
                                     _md['moe_frozen_qx_coef']) if _pd['prompt_len'] > 0 and _pd['prompt_start_block'] <= i <= _pd['prompt_end_block'] else None)
        ...
        _logit_scale: float = other_args_dict['logit_scale']  # 4.605170249938965
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            for _b, _blk in enumerate(self.blocks):
                if 'q_x' in locals() and hasattr(_blk, 'moe_prompt'):
                    x = (x, q_x)
                x = _blk(x)
        x = self.norm(x)
        return x
    ...
    def forward_head(self, x: torch.Tensor, pre_logits: bool = False) -> torch.Tensor:
        if self.attn_pool is not None:
            ...
            assert x.shape[1] == 197, f"{x.shape}"
            x = x[:, 0]  # class token
        else:
            raise NotImplementedError()
        x = self.fc_norm(x)
