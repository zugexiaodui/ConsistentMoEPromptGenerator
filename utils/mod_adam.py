            ...
            if _l2 == 0:
                ...
                for _up, _dp in zip(updates, device_params):
                    if (_pid := id(_dp)) in update_projection_dict:
                        ...
                        if 'interm_reader_1' in update_projection_dict[_pid]:
                            _proj = update_projection_dict[_pid]['interm_reader_1']
                            _up.data = torch.matmul(_proj, _up).data
                        elif 'interm_reader_2' in update_projection_dict[_pid]:
                            _proj = update_projection_dict[_pid]['interm_reader_2']
                            _up.data = rearrange(torch.matmul(_proj, rearrange(_up, 'M L D -> M (L D)')), 'M (L D) -> M L D', M=_up.shape[0], L=_up.shape[1], D=_up.shape[2]).data
